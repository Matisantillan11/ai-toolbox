...
function sanitizeMeta(meta = {}) {
  ...
  for (const [key, value] of Object.entries(meta)) {
    if (["decision", "reasoning", "stack", "term", "content", "payload"].includes(key)) {
...
export function logEvent(level, event, meta = {}) {
  if (process.env.AI_TOOLBOX_NKN_SILENT === "1") {
    return;
  }
...
/**
 * Writes an informational NKN log event.
 *
 * @param {string} event Event name.
